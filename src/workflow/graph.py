...
def load_document_node(state: IngestionState) -> IngestionState:
    """Load document from file path, preserving page structure.

    Args:
        state: IngestionState with file_path and document_metadata.

    Returns:
        Updated state with document_pages (per-page docs) and status='loaded'.
    """
    docs = load_document(state["file_path"], state["document_metadata"])

    return {
        **state,
        "document_pages": docs,  # Preserve page-level structure
        "status": "loaded",
    }


def chunk_document_node(state: IngestionState) -> IngestionState:
    """Split documents into structurally-chunked pieces, preserving page structure.

    Args:
        state: IngestionState with document_pages loaded.

    Returns:
        Updated state with chunks and status='chunked'.
    """
    chunker = StructuralChunker()
    chunks = chunker.split_documents(state["document_pages"])
...
def create_prd_review_graph() -> StateGraph:
    ...
    # Re-retrieve loops back to all 4 validators (refinement loop per D-17)
